...
When this line gets executed (which actually happens even before the link:/reference/en/language/structure/sketch/setup/[`*setup()*`] function), the constructor for the Morse class will be called, and passed the argument you've given here (in this case, just *14*).

Notice that our link:/reference/en/language/structure/sketch/setup/[`*setup()*`] is now empty; that's because the call to link:/reference/en/language/functions/digital-io/pinmode/[`*pinMode()*`] happens inside the library (when the instance is constructed).
