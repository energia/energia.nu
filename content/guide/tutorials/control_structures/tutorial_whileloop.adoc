...
Sometimes you want everything in the program to stop while a given condition is true. You can do this using a http://energia.nu/reference/while/[*while loop*]. This example shows how to use a while loop to http://energia.nu/guide/tutorial_calibration/[calibrate] the value of an analog sensor.
...
* http://energia.nu/guide/tutorial_switchcase/[Switch Case]:choose between a number of discrete values in a manner that is the equivalent of using multiples If statements. This example shows how to divide a sensor's range into a set of four bands and to take four different actions depending on which band the result is in.
